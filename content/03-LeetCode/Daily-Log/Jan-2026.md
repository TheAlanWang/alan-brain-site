...
#### Leetcode
###### LC1. Two Sum
Solve Type: Answer, Do in 10mins
1. **Let me restate the problem,** give an integer array `nums` and an int `target`, need we to return the indices of the two numbers, sum number as same as target.
	Each input has exactly one solution, may not use the same element twice.
2. **The key idea:** iterate the array. For each number x, I compute `target - x`. If complement is already in hash map. I return the stored index and the current index. Otherwise, I store x->index in the map. 
3. **Time complexity** is O(n), Space Complexity is O(n)
4. **Let me test an edge case**, duplicate number or no solution
###### LC1636. Sort Array by Increasing Frequency
Solve Type: Solution
1. **Let me restate the problem**, give an integer array `nums`, sort the array so that lower frequency comes at first. if frequency is the same, return the larger value. Return the sorted array.
2. **The key idea:** Count the frequency of each number with a hash map / `Counter`, then sort `nums` using a custom key: `(frequency[x], -x)`.
3. **Time complexity** is O($nlogn$), Space Complexity is O(n)
4. **Edge case:** Negative numbers / zero
