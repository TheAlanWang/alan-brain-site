...
4. Quit the `Dev` role and now assume the `User` role, and do the following

 -  Find all the objects whose key has the prefix `assignment` and compute the total size of those objects.

5. Quit the `User` role and now assume the `Dev` role, then delete the all the objects and the bucket.
